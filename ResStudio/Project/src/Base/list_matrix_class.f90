...
module list_matrix_class
	...
	public :: list_matrix_get_matrix, list_matrix_get_handle, list_matrix_get_vars,list_matrix_get_name
...
contains
		...
		do
			if (.not. associated(cur)) then
				...
				exit
                        ...
                        !write (*,*) "Matrix scan:",nam,cur%nam,(cur%nam == nam) 
			if (cur%nam == nam) then
				res = cur%handle
				exit
			else
	...
	function list_matrix_get_name(this, handle) result(res)
		type(list_matrix), intent(in) :: this
		character(len=31)  :: res
		type(tl_handle) ,intent(in):: handle	
		type(list_matrix_node), pointer :: cur
                res = ""
		cur => this%first
		do
			if (.not. associated(cur)) then
				write(*,*) "Error: Could not return name, matrix with handle '", handle%address, "' not found."
				exit
			end if
		
			if (cur%handle%address == handle%address) then
				res = cur%nam
				exit
			else
                                !write (*,*) "h ",cur%handle%address
				cur => cur%next
			end if
		
		end do
	
       end function list_matrix_get_name

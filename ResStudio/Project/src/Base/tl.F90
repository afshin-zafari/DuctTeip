...
module tl
  ...
  interface
#ifdef LINUX 
     subroutine tl_add_task_named(tname,fptr,arg,sz,hr,nhr,hw,nhw,ha,nha) ,bind(C)
       use tl_handle_class
       use ISO_C_BINDING
       character(kind=C_CHAR) , intent(in) :: tname(*)
       type(C_FUNPTR), intent(in), value :: fptr
       integer(C_INT) , intent(in) :: sz,nhr,nhw,nha
       integer , dimension(0:nhr-1),intent(in) ::hr
       type(tl_handle), dimension(1:nhw),intent(in) ::hw
       integer , dimension(0:nha-1),intent(in) ::ha
       !type(C_PTR)  ::hr,hw,ha

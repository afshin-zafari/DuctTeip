...
# 136
    ...
    Function dist_large_prob(this) Result(answer)
        ...
        call rma_initialize(this%opt,this%wf,this%rank,part_cnt=pc,part_size=10,node_cnt=nc,dim_cnt=2,np=np,nt =nt )
        answer = .True.
...
# 423
	...
	Type(distribution),Intent(inout) :: this
        Integer 			 :: err
        character(len=STR_MAX_LEN)::sch_dir

        sch_dir=opt_get_option_str(this%opt,STR_OPT_SCHED_DIR)//char(0)
	call tl_destroy(sch_dir)

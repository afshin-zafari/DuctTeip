...
def create_task(t,host,r,c):
    ...
    if t == "Diag":
        ...
        return s1
    if t == "MAdd":
        gtask_list.append([ [MAdd,r,c,host,2]  , [DX,r,c-1,-1,-1,0,-1], [DM,r,c,-1,-1,0,-1], [DM,r,c,-1,-1,0,-1], -1,-1,-1,-1] )
        s1= s+"{0}@{1}  X({2},{3})  M({2},{4})  M({2},{4})  2".format ( t,host,r,c-1,c)+'\n'        
        return s1
    if t == "PnlU":
        ...
        if r-1==c :
            gtask_list.append([ [MMul,r,c,host,4]  , [DM,r,c,-1,-1,0,-1], [DM,r,c,-1,-1,0,-1], [DY,r,c,-1,-1,0,r], -1,-1,-1,-1] )
            s1+= s+"{0}@{1}  M({2},{3})  M({4},{3})  X({2},{3},{4},{3})  4".format ( t,host,r,c,r)+'\n'            
            gtask_list.append([ [XAdd,r,c,host,1]  , [DY,r,c,-1,-1,0,r], [-1,r,c,-1,-1,-1,-1], [DX,r,c,-1,-1,0,-1], -1,-1,-1,-1] )
            s1+= s+"X -=@{1}  X({2},{3},{4},{3})  --  X({2},{5})  1".format ( t,host,r,c,r,c)+'\n'
        return s1
    if t == "MMul":
        s1=""
        r1=0
        if 0 : 
            for c1 in range(c,-1,-1):
                gtask_list.append([ [MMul,r,c,host,4]  , [DM,r,c1,-1,-1,0,-1], [DM,c1+1,c1,-1,-1,0,-1], [DY,r,c1,-1,-1,0,c1+1], -1,-1,-1,-1] )
                s1+= s+"{0}@{1}  M({2},{3})  M({4},{3})  X({2},{3},{4},{3})  4".format ( t,host,r,c1,c1+1)+'\n'            
                gtask_list.append([ [XAdd,r,c,host,1]  , [DY,r,c1,-1,-1,0,c1+1], [-1,r,c,-1,-1,-1,-1], [DX,r,c,-1,-1,0,-1], -1,-1,-1,-1] )
                s1+= s+"X -=@{1}  X({2},{3},{4},{3})  --  X({2},{5})  1".format ( t,host,r,c1,c1+1,c)+'\n'
        else:
            for r1 in range(r-1,c,-1):
                ...
                gtask_list.append([ [XAdd,r,c,host,1]  , [DY,r,c,-1,-1,0,r1], [-1,r,c,-1,-1,-1,-1], [DX,r,c,-1,-1,0,-1], -1,-1,-1,-1] )
                s1+= s+"X -=@{1}  X({2},{3},{4},{3})  --  X({2},{5})  1".format ( t,host,r,c,r1,c)+'\n'
            print r,c,r1,'\n',s1
        return s1
...
def experiment(print_stat_all,print_stat_part,print_comm_cost,print_comm_bal,print_final,comp_load_balance,comm_load_balance,n,p):
    global gtask_list,dax_list    
    v = range(0,10)
    for part in [p]:
        p = part
...
if len(sys.argv)>3:
    loadbal = int ( sys.argv[3])
if len(sys.argv)>4:
    commpipe= int ( sys.argv[4])
if len(sys.argv)>5:
    printfinal= int ( sys.argv[5])
if len(sys.argv)>6:    
    tfile=  sys.argv[6]
    run_time_stat(tfile)
else:
    experiment(0,0,commpipe,commpipe,printfinal,loadbal,commpipe,n,p)

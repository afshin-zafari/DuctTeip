...
#-------------------------Communication Cost ( Message Count ) ----------------------------------
...
cs_cctot   = [d[CcTot_COL] for d in cs ]

cp = list()
for i in range(0,len(dt_cctot)):
    cp.append( dt_cctot[i] / cs_cctot[i] ) 
ovh = int((sum(cp) / len(cp) - 1.00)*100)
print ovh,cp
for cc in dt_cc:
    print cc
...
cs_cctot   = [d[CcSize_COL] for d in cs ]

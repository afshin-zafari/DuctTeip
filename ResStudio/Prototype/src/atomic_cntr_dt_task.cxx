...
/*--------------------------------------------------------------*/
void IDuctteipTask::addToChildren(){
  Atomic::increase(&child_count);
}
/*--------------------------------------------------------------*/
void IDuctteipTask::removeFromChildren(){
  LOG_INFO(LOG_TASKS,"Before.\n");
  while ( isSubmitting ){}
  LOG_INFO(LOG_TASKS,"After.Chld:%ld\n",child_count);
  if ( Atomic::decrease_nv(&child_count) ==0){
    //  if(--children<0){
    setFinished(true);
    LOG_INFO(LOG_TASKS,"DTTask finished %s\n",getName().c_str());
  }
}
/*--------------------------------------------------------------*/
void IDuctteipTask::subtask(SuperGlueTaskBase *t){
  ...
  if ( config.pure_mpi){
    ...
    printf("scheduled task %s, child-cnt:%ld.\n",t->get_name().c_str(),child_count);
#endif
  }
}
/*===============================================================*/
SuperGlueTaskBase::SuperGlueTaskBase(IDuctteipTask* d):dt_task(d){
    ...
    #ifdef ATOMIC_CNTR
    if (dt_task)
      dt_task->addToChildren();
    #endif
    LOG_METRIC(DuctteipLog::SuperGlueTaskDefine);
}
/*--------------------------------------------------------------*/
SuperGlueTaskBase::~SuperGlueTaskBase(){
#ifdef ATOMIC_CNTR
  if ( dt_task)
    dt_task->removeFromChildren();
  LOG_INFO(LOG_TASKS,"SGTask finished: %s\n",name.c_str());
#endif
}
/*--------------------------------------------------------------*/

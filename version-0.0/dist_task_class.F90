...
!!$  ------------------------------------------------------------------------------------------------------------
  Subroutine task_add_to_list(this,task)
    ...
    Else
    End If
    call instrument(EVENT_TASK_ADDED,i,0,0)
...
!!$  ------------------------------------------------------------------------------------------------------------
  ...
  Subroutine task_can_run(this,task,event,answer)
    ...
    call instrument(EVENT_TASK_CHECKED,task%id,0,0)
      ...
      Case  (TASK_STS_INPROGRESS)         
         If ( Associated(task%dmngr) ) Then 
            If ( event == EVENT_TASK_FINISHED .Or. dm_get_sync(task%dmngr)> 0  ) Then 
               ...
               write(*,*) event_name(EVENT_TASK_FINISHED),',',(MPI_Wtime()),',',EVENT_TASK_FINISHED,',',task%id,',',task%name,',',t&
&ask&
# 187
%weight
    ...
    If (to_sts /= from_sts .Or. notify /= NOTIFY_DONT_CARE) Then 
       If (task%id /= TASK_INVALID_ID) Then 
          call instrument(EVENT_TASK_STS_CHANGED,task%id,0,0)
       End If
    End If
...
!!$  ------------------------------------------------------------------------------------------------------------
  ...
  Subroutine task_clean(this,task,event)
    ...
    call instrument(EVENT_TASK_CLEANED,task%id,0,0)
    write(*,*)  "Task Cleaned"," : ",task%name
...
!!$  ------------------------------------------------------------------------------------------------------------
  ...
  Subroutine task_match_access(this, task_id,axs_id, answer)
    ...
    If ( Associated(this%task_list(task_id)%axs_list(axs_id)&
# 397
                       %data) ) Then 
       data => this%data_list(this%task_list(task_id)%axs_list(axs_id)&
# 398
                                 %data%id)
       If (this%task_list(task_id)%axs_list(axs_id)&
# 399
              %data%proc_id /= this%proc_id) Then           
          if ( data%has_lsnr == 0 ) then              
             lsnr%req_data=>data
             lsnr%proc_id=this%task_list(task_id)%axs_list(axs_id)&
# 402
                             %data%proc_id
             lsnr%dname = data%name
             lsnr%min_ver=this%task_list(task_id)%axs_list(axs_id)&
# 404
                             %min_ver
             data%has_lsnr = 1
          ...
          End If
       End If
       If ( (data%status == DATA_STS_READ_READY   .And. ( mod(this%task_list(task_id)%axs_list(axs_id)%access_types,AXS_TYPE_READ*1&
&0&
# 410
)/AXS_TYPE_READ ) /= 0                                      ) &
            .Or. &
            (data%proc_id == this%proc_id .And. ( mod(this%task_list(task_id)%axs_list(axs_id)%access_types,AXS_TYPE_WRITE*10)/AXS_&
&TYPE_WRITE&
# 412
 ) /= 0                                              ) &
             .Or. &
             (data%status == DATA_STS_MODIFY_READY   .And. ( mod(this%task_list(task_id)%axs_list(axs_id)%access_types,AXS_TYPE_REA&
&D&
# 414
*10)/AXS_TYPE_READ ) /= 0                                      ) &
             ) Then 
          If (data%version >= this%task_list(task_id)%axs_list(axs_id)&
# 416
                                 %min_ver) Then 
             answer = .True.
             this%task_list(task_id)%axs_list(axs_id)&
# 418
                %match_status = DATA_STS_READ_READY
          End If
       End If
    Else
       d1 = data_find_by_name_dbg(this,this%task_list(task_id)%axs_list(axs_id)%dname,"dist_task_class.F90",482)
       If ( d1%id == DATA_INVALID_ID ) Then 
          Data=>data_create(this,this%task_list(task_id)%axs_list(axs_id)&
# 424
                                    %dname,this%np,this%nd,this%task_list(task_id)%axs_list(axs_id)&
# 424
                                                              %min_ver,this%task_list(task_id)%axs_list(axs_id)&
# 424
                                                                          %dproc_id)          
          d1 = data_find_by_name_dbg(this,this%task_list(task_id)%axs_list(axs_id)%dname,"dist_task_class.F90",486)
       End If
...
!!$-------------------------------------------------------------------------------------------------------------------
  Subroutine task_rbf_mat_purempi(this , taskid)
    ...
    write(*,*) event_name(EVENT_TASK_STARTED),',',(MPI_Wtime()),',',EVENT_TASK_STARTED,',',task%id,',',0,',',0
    ...
    write(*,*) event_name(EVENT_TASK_FINISHED),',',(MPI_Wtime()),',',EVENT_TASK_FINISHED,',',task%id,',',0,',',0
...
!!$  ------------------------------------------------------------------------------------------------------------
  Subroutine task_rbf_mat(this,taskid)
    ...
    write(*,*) event_name(EVENT_TASK_STARTED),',',(MPI_Wtime()),',',EVENT_TASK_STARTED,',',task%id,',',0,',',0
    call instrument(EVENT_TASK_STARTED,task%id,this%task_cnt,0)
    call instrument(EVENT_TASK_RUNNING,task%id,this%task_cnt,0)
...
!!$------------------------------------------------------------------------------------------------------------
  ...
  Subroutine task_run(this,task) 
    ...
    Select Case(task%name)
       Case (TNAME_ASM_MAT)
          purempi = opt_get_option(this%opt,OPT_PURE_MPI)
          If (purempi == 1 ) Then
              Call task_rbf_mat_purempi(this,task%id)
          Else
              Call task_rbf_mat(this,task%id)
          End If 

       Case(TNAME_CHOL_DIAG,TNAME_CHOL_XADD,TNAME_CHOL_MADD,TNAME_CHOL_PNLU,TNAME_CHOL_MMUL)
          write(*,*) event_name(EVENT_TASK_STARTED),',',(MPI_Wtime()),',',EVENT_TASK_STARTED,',',task%id,',',task%name,',',task%wei&
&ght
# 587
          Call task_chol_local(this,task)
          event = EVENT_TASK_STARTED
          task = this%task_list(task%id)
          Call task_check_sts(this,task,event,notify)
          this%task_list(task%id) = task
          event = EVENT_TASK_RUNNING
          Call task_check_sts(this,task,event,notify)
       ...
       Case (TNAME_INIT_DATA) 
          dname = task_write_name(this,task%id)
          Data = data_find_by_name_dbg(this,dname,"dist_task_class.F90",667)
          If ( data%id == DATA_INVALID_ID ) Then 
             Data = data_create(this,dname,this%np,this%np,0,this%proc_id)
          End If
          Call data_populate(this,dname ,pts)
          this%task_list(task%id)%axs_list(1)%data =>this%data_list(data%id)
          this%task_list(task%id)%status = TASK_STS_FINISHED
          task = this%task_list(task%id)          
          Call task_upgrade_data(this,task)
          Call print_lists(this)

       Case ("NoTask")
          write(*,*)  "DuctTeip Task Runs"," : ",task%name
          if (task%name == TNAME_CHOL_XADD) tw = 1
          if (task%name == TNAME_CHOL_MADD) tw = 2
          if (task%name == TNAME_CHOL_DIAG) tw = 3
          if (task%name == TNAME_CHOL_MMUL) tw = 4
          if (task%name == TNAME_CHOL_PNLU) tw = 5

       Case DEFAULT
          Call task_sample(this,task%id)
...
!!$------------------------------------------------------------------------------------------------------------
  Subroutine task_chol_local(this,task)
    Type(remote_access) , Intent(inout)    :: this
    ...
    allocate ( chol_obj%dobj1 , chol_obj%dobj2, chol_obj%dobj3)
    allocate ( chol_obj%dobj1%dmngr , chol_obj%dobj2%dmngr, chol_obj%dobj3%dmngr)
    call dm_new( chol_obj%dobj1%dmngr)
    call dm_new( chol_obj%dobj2%dmngr)
    call dm_new( chol_obj%dobj3%dmngr)



!!$    Task Read1   Read2   Write      Parameters
!!$    ---- -----   -----   -----  -------------------
!!$    XADD   A      --      B      rows,cols,dmngr,nb
!!$    MADD   A       B      B             " 
!!$    PNLU Diag      A      A             "
!!$    DIAG   A      --      A             "
!!$    MMUL   A       B      C             "
    ...
    ! Rows and Cols
    chol_obj%rows = this%np
    chol_obj%cols = this%np
    ...
    chol_obj%dobj3 => this%data_list(wrdata%id)
    If (this%data_list(wrdata%id)%version == 0 ) Then 
       chol_obj%data3(:,:)= 0
    End If
    !  Read1 Data
    ...
    chol_obj%dobj1 => this%data_list(r1data%id)
    
    

    If ( task%name /= TNAME_CHOL_XADD .And. task%name /= TNAME_CHOL_DIAG) Then 
       !  Read2 Data
       ...
       chol_obj%dobj2 => this%data_list(r2data%id)
    ...
    Select Case ( task%name)
        Case (TNAME_CHOL_XADD) 
           chol_obj%nb = this%nc
           Call chol_mat_xadd_task ( chol_obj )
        Case (TNAME_CHOL_MADD) 
           chol_obj%nb = this%nc
           Call chol_mat_add_task  ( chol_obj )
        Case (TNAME_CHOL_PNLU) 
           chol_obj%data2 => this%data_list(r1data%id)%mat
           chol_obj%dobj2 => this%data_list(r1data%id)
           chol_obj%data1 => this%data_list(r2data%id)%mat
           chol_obj%dobj1 => this%data_list(r2data%id)
           this%data_list(wrdata%id)%mat =>this%data_list(r2data%id)%mat
           chol_obj%nb = this%nc
           Call chol_mat_pnlu_task ( chol_obj )
        Case (TNAME_CHOL_DIAG) 
           this%data_list(wrdata%id)%mat =>this%data_list(r1data%id)%mat
           chol_obj%nb = this%nc
           if (opt_get_option(this%opt,OPT_CHOL_CACHEOPT) /=0) then 
              call chol_diag_task_cache_opt ( chol_obj )
           else
              call chol_diag_task ( chol_obj )
           end if
        Case (TNAME_CHOL_MMUL) 
           chol_obj%nb = this%nc
           Call chol_mat_mul_task  ( chol_obj )
    End Select

    
  End Subroutine task_chol_local
!!$------------------------------------------------------------------------------------------------------------
  ...
  Subroutine task_upgrade_data(this,task)
    ...
    Do i=Lbound(task%axs_list,1) , Ubound(task%axs_list,1)
       ...
       If (task%axs_list(i)%data%id /= DATA_INVALID_ID) Then 
          Data = data_find_by_name_dbg(this,task%axs_list(i)%data%name,"dist_task_class.F90"&
# 802
                                                                               ,802     )
          If (data%id /= DATA_INVALID_ID) Then 

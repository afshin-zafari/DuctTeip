...
Public :: new_pts, destruct_pts, copy_pts, get_pts_ptr, get_pts_id, &
          ...
          read_pts_part,get_ids_ptr,pts_get_info,get_pts_sids,add_pts
...
!!$------------------------------------------------------------------------------------------------------------------------------
  Subroutine pts_get_info(fname,np,nd,nt) 
...
!!$------------------------------------------------------------------------------------------------------------------------------
 Function add_pts(p,pts) Result(succeed)
    ...
    If ( .Not. Associated(in_ids) ) Then 
        write(*,*)  "add_pts"," : ","Exit,no in ids"
        Return 
    End If 

    If ( .Not. Associated(in_ptr) ) Then 
        write(*,*)  "add_pts"," : ","Exit,no in ptr"
        Return 
    End If 
    ...
    write(*,*)  "max_id,pos"," : ",(max_id,ids_from)
    write(*,*)  "ptr_from,to,cnt"," : ",(ptr_from,ptr_to,ptr_cnt)

    If (ptr_to > Ubound( this_ptr,1) .Or. ptr_cnt > Ubound(in_ptr,1) ) Then 
        write(*,*)  "this p#"," : ",Ubound( this_ptr,1)
        write(*,*)  "add_pts"," : ","Exit,idx out of range"
        Return 
    End If 
    ...
    Do i = 1, Ubound(in_ids,1)
        ...
        write(*,*)  "ADDED "," : ",(ids_from,this_ids(ids_from,:) )
...
!
    sp(1) = Minval(pts%id%i%i2(:,1),pts%id%i%i2(:,ID_COL_FROM).ne.0)
    sp(2) = Maxval(pts%id%i%i2(:,1),pts%id%i%i2(:,ID_COL_FROM).ne.0)
